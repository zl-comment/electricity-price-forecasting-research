...
**修订说明（2026-09-14，执行前先落实；下文第 3.2、3.4、4.1 节的相应细节须随之修订）**：

| 项 | 下文原写法 | 已决定的新口径 | 依据 |
|---|---|---|---|
| FB2 场景输入 | S3-A 的 `fb2_gate` 等 `scenarios_0730/`、`scenarios_1200/` | `own_pooled` 边际配经验 Copula 与高斯两种耦合，均进入 B3；`p1_paper/results/s3_cross_market/` 未导出场景文件，须先补导出 | 执行计划第 10.5 节 |
| 12:00 产品 | FB2 精确高斯条件化、经验 Copula 类比条件化 | 至少比较无条件、高斯 24 小时条件化、直接分位数回归三种 | 执行计划第 10.5 节 |
| X14 重建 | 决定形状的字段未定且非明示假设时跳过 | 按来源卡已核读内容与明示假设实现最接近变体，逐条记录偏差，不跳过 | 2026-09-14 用户决定 |
| RQ4 | 未单列 | 预测评分与决策排名不一致不作主张，只报告复验 | 论文规划第 6 节 |
...
某字段「未确定」且决定决策形状时：论文明示假设则实现最接近变体并记偏差，否则该臂不跑，写入 `summary.json` 的 `skipped_arms`。X14 为例外，见开头修订说明。
