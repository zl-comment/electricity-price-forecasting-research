...
- QuantReg 的 `tolerance` 与 `max_iter` 写 yaml；取值偏离 statsmodels 默认时在 `deviations` 记明，并报告达到迭代上限的格数——达到上限仍然报错停下。τ = 0.005 与 0.01 低于 84 天拟合窗的分辨率 1/84 ≈ 0.0119，尾部指标中须标明这两档是外推而非估计。
