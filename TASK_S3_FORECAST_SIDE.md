...
`ForecastDayAhead` 的原始缺失只影响这一条臂，处理办法预先规定如下，不适用于其他任何臂。2026-05-07、2026-05-08 两个测试日的海上风整日缺失，2025-11-09、2025-11-24 三类风光整日缺失但都不是测试日。拟合与校准块内的缺失行按第 3.5 节照常剔除并计数，不构成停止条件；`fb2_plus_weather` 的评价日集合减为 209 天，缺的两天写入 `summary.json` 的 `arm_day_coverage.fb2_plus_weather`，列出日期与缺失的 `ForecastType`。这一条不得用于任何其他臂：其余臂一律在 211 天上评价，测试日特征缺失仍是断言失败。

因此 `fb2_plus_weather` 的所有对比只在 209 天的公共子集上做，与之比较的对照臂同样限制到这 209 天，表中标注天数；主表 211 天的数字不得因这条臂发生任何变化，两套数字并列呈现。导出场景时该臂仍写满 (211, 50, 4, 24)，缺的两天整片写 NaN，另存 `evaluated_days` 数组标明真正参与评价的 209 天；NaN 是显式缺失标记，不是补值，读取方必须按 `evaluated_days` 跳过。
...
- 激活量的退化边界（预先规定，不是后备分支）：某小时的 F(D) 内激活标签只有正值时不拟合 Logistic，取 p0 = 0，正值 QuantReg 照常拟合。yaml 写 `degenerate_zero_rule: p0_zero_when_no_zero_events`，yaml 的 Logistic 参数须保持截距不参与惩罚，使 p0 = 0 就是该情形下惩罚极大似然的极限解。反向情形（F(D) 内只有零事件）不适用本规则，断言并报错停下。
  触发的每个（测试日，小时）逐格写入 `panel_audit.csv`，同时写入该格 F(D) 的零事件个数；`summary.json` 的 `deviations` 记总格数、涉及的测试日数与小时集合。零事件 Brier 与覆盖率须同时报告全部格子与剔除退化格子后的两套数值。`panel_audit.csv` 另报全部（测试日，小时）上 F(D) 零事件个数的分布，使零事件只有 1、2 个的准分离格子可见。
- 分位点之间线性插值；[0.005, 0.995] 之外取端点值，并在尾部指标中报告这一截断。
- Logistic 与 QuantReg 共用同一套拟合窗 z-score，`predictor_scaling: fit_window_zscore` 对两者同时生效。sklearn 的 L2 惩罚随特征量纲变化：天气臂的风光日前预测以 MW 计（陆上风最大约 3000），而零值占比与周末哑变量在 0–1 之间，原始量纲下惩罚对大尺度特征几乎不起作用，线性预测值可以在惩罚代价接近于零的情况下达到 ±36，p0 于是数值上取到恰好 1.0 或 0.0。z-score 后两类特征的惩罚才可比，第 4.2 节「截距不参与惩罚使 p0 = 0 是极限解」的论证也才成立。
- 混合分位函数按公式惰性求值：p0 ≥ 最大分位点时整行直接输出 0，不计算正值指数分支。这是公式本身，不是后备分支。
- `summary.json` 记录 p0 的分位数摘要，以及 p0 ≥ 0.995 与 p0 ≤ 0.005 的格数；两者都不为零时在 `deviations` 说明。
- QuantReg 的 `tolerance` 与 `max_iter` 写 yaml；取值偏离 statsmodels 默认时在 `deviations` 记明，并报告达到迭代上限的格数——达到上限仍然报错停下。
- 可估计分位区间：每个 QuantReg 拟合单元按其有效样本数 n（完整行数；激活量正值部分为正值完整行数）确定可估计区间 [1/(n+1), 1−1/(n+1)]，**只在落入该区间的分位点上拟合**。区间之外的分位点不拟合，按既有 `tail_policy: endpoint` 取最靠内的已估计分位点之值。每个单元的 n、被替代的分位点写入 `panel_audit.csv`，总格数与涉及的分位点写入 `summary.json` 的 `deviations`。
  理由：84 天窗的分辨率约为 1/84 ≈ 0.0119，τ = 0.005、0.01 与多数小时的 τ = 0.99、0.995 都落在样本分辨率之外，此处的线性规划由不足一个观测决定，解是退化顶点而非尾部估计。实测 D = 2026-03-20、hour 7、weather=True 的正值单元有 81 个正值样本，log 激活量实际范围为 [−3.507, 4.347]，而 τ = 0.005 的拟合给出 log 预测 1160.86，是实际最大值的 267 倍——这是 LP 崩解，不是重尾。
  后果必须如实报告：激活量正值部分的 q99 在多数小时等于 q97.5 的估计值，q99 超越频率会偏高。这是 84 天窗的固有限制，对全部臂一视同仁，不影响臂间比较，在尾部指标与研究文档中写明「该档为分辨率外的端点延拓，非估计」。
- 不得把正值分位预测裁剪到 F(D) 正值标签的 [min, max]：上端裁剪会把 q95、q99 封死在 84 天的历史最大值内，使尾部指标测的是裁剪规则而不是模型，同时掩盖分辨率内的系数爆炸。指数越界的断言保留为报错——改用可估计区间后它不应再触发，触发即停下报告。
- 预测变量的可识别性与支撑，两步，都只看 F(D)：（一）剔除本窗口无法识别的列——某拟合单元内该预测变量的标准差，低于同一窗口内该预测变量跨小时合并标准差的 `predictor_variation_floor` 倍时剔除该列，其余列照常 z-score；（二）预测时把标准化后的协变量裁剪到拟合窗观测范围（`predictor_support_clip: fit_window_range`）。裁剪的是协变量，不是目标分位值，q95、q99 仍可超过历史标签最大值，与上一条不冲突。
  理由：z-score 的分母在季节性休眠的特征上会塌缩。实测 D = 2026-03-20、hour 7 的 F(D) 是 2025-11-21 至 2026-02-12，该小时光伏日前预测均值 0.032 MW、标准差 0.116 MW（冬季天未亮），而交割日该小时为 269.99 MW，标准化后得 2350，远在拟合支撑之外，τ = 0.025 的 log 预测因此到 1138。全 211 天扫描共有 1732 个（日，小时，特征）槽位的标准化值绝对值超过 20，最大 1.86e6，另有 81 个槽位的零值占比特征在窗口内完全无变化。窗口内没有变化的特征，其系数不可识别；线性模型在支撑之外也没有可说的东西，无界外推进 exp 不是尾部估计。
  记录：每个拟合单元被剔除的列、发生裁剪的条目数、标准化协变量绝对值的最大值写入 `panel_audit.csv`，汇总与按特征的剔除计数写入 `summary.json` 的 QuantReg 诊断块。
...
立即停下并报告：
- 测试日集合不等于 S2 的 211 天（`fb2_plus_weather` 按第 4.1 节评价 209 天是已授权的例外），或任一可见性断言失败；
