...
基线：`main` = `2083108`。环境固定为 `/public/ZLCODE/.venvs/r1_lear_de/bin/python`。本文件在两个任务都结束后删除。
...
### 0.4 划分是完全确定的（此前判为「论文笔误」，那是错的）

`extended_data_v2.csv` 覆盖 2024-06-20 22:00 至 2025-03-16 22:00 UTC。完整小时网格是 **6,457 点**，而文件只有 6,456 行——缺的一点在 **2024-10-27 01:00 UTC**（欧盟夏令时回拨日）。

论文第 6.1 节说的「共 6457 条」指的正是这个完整网格，不是笔误。由此划分可精确重建：

```
6457（完整小时网格）
 − 24（头部，24 小时滚动特征不可用）
= 6433
= 4497 训练 + 968 验证 + 968 测试        （70/15/15）
→ 从测试挪 8 点到验证
= 4497 / 976 / 960                      （测试 960 = 20 × 48）
```

**执行时必须先把数据重索引到 6,457 点的完整小时网格再切分**，否则划分边界会整体错一格。缺的那一点落在训练段内（网格第 3,074 位），保持为 NaN 即可——LightGBM 原生支持缺失值，不需要做任何填补选择。

### 0.5 由此定级：部分复现，两个自由参数 ＋ 一条继承边界

数据、划分、指标、种子四项可精确对齐。另有三条边界，**必须分两类记录，不要混成一句「三条限制」**：

| 类别 | 内容 | 是否拉大我们与论文的差距 |
|---|---|---|
| **自由参数**（我们必须替论文做的选择） | 特征子集 | **是** |
| | LightGBM 超参 | **是** |
| **继承边界**（我们没有做选择） | 15 分钟填补与小时聚合无法本地核验 | **否** |

前两条是论文没写、我们只能自己定的，它们**确实**让我们的数字和 17.58 / 55.74 之间多出无法归因的差距。

第三条性质相反。Zenodo 压缩包 `afRR_energy_market_hourly_Data.zip` 只含 `extended_data_v1.csv` 与 `v2.csv` 两份**已聚合的小时数据**，没有原始 15 分钟序列，v2 也没有任何 NaN——**作者发布的就是他们自己预处理后的成品**。我们直接用它，等于继承了论文的预处理，**没有引入任何自己的选择，因此不产生分歧**。若反过来从 Fingrid 重新采集 15 分钟原始数据自行填补聚合，那才是引入我们的选择，离原文更远。

所以结果文件里第三条要写成**来源声明**（`preprocessing: inherited_from_authors_published_dataset`），不能和前两条并列成「第三个自由参数」——那会低估这次复现的质量。它唯一的真实含义是：我们无法独立核验作者的预处理是否正确，仅此而已。

综上，**R2 是「部分复现」，不是严格复现**：我们与 17.58 / 55.74 的差距只能归因到特征子集与超参两项。这句限制必须写进结果文件。
...
| 缺失值与聚合 | **不做**。Zenodo 压缩包（`afRR_energy_market_hourly_Data.zip`）只含两份已聚合的小时 CSV，无原始 15 分钟序列，v2 亦无任何 NaN。作者发布的就是成品，直接用。见 0.5 的第三条边界 |
| 时间网格 | 先重索引到 6,457 点完整小时网格（见 0.4），缺的 2024-10-27 01:00 UTC 保持 NaN |
| 异常值 | **不剔除**，前后都不做 |
...
9. 不要自行从 Fingrid 重新采集 15 分钟原始数据来复刻作者的填补与聚合——那是引入我们自己的选择，离原文更远，不是更近。
