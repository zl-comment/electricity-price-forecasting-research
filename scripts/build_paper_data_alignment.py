...
ALIGNMENT = {
    ...
    "X13": ("original_belgian_market_sample_not_local", [relation("DS1_DK1_MULTI_MARKET", "delivery_risk_constraint_transfer", "DK1 activation volumes support re-estimating delivery-risk constraints under the local protocol; a nominal joint chance constraint is not a temporal out-of-sample guarantee.")]),
    ...
    "F31": ("paper_time_series_benchmarks_not_local", [relation("DS1_DK1_MULTI_MARKET", "online_predictive_coverage_transfer", "Transfers the feedback-control structure for time-average predictive miscoverage; the paper does not test storage non-delivery loss or delayed decision feedback.")]),
    ...
    "F34": ("original_open_system_data_not_local", [relation("DS3_RTS_GMLC", "joint_forecast_reserve_allocation_transfer", "The public system-level joint forecasting and reserve-allocation workflow can transfer to a transparent system benchmark; the paper's original data are not mirrored locally.")]),
    "F35": ("foundation_model_pretraining_and_benchmarks_not_local", [relation("DS4_EPF_DE", "probabilistic_foundation_model_benchmark", "EPF-DE can test PriceFM under the project protocol after a pretraining-contamination audit; this is not a paper-result reproduction.")]),
    "F36": ("paper_market_panels_not_packaged_locally", [relation("DS4_EPF_DE", "forecast_to_battery_value_benchmark", "EPF-DE supports a reduced forecast-versus-arbitrage-value comparison, not the preprint's full market panel.")]),
    "F37": ("paper_price_benchmarks_not_packaged_locally", [relation("DS4_EPF_DE", "online_conformal_price_transfer", "Transfers online conformal recalibration to a pinned day-ahead price benchmark; original paper result tables are not reproduced.")]),
    "F38": ("generic_delayed_feedback_method_benchmarks_not_local", [relation("DS1_DK1_MULTI_MARKET", "delayed_predictive_calibration_transfer", "The delay-to-memory analysis informs feedback indexing, but its predictive-coverage guarantee does not directly establish control of storage decision loss.")]),
    ...
    "M08": ("original_danish_wind_hess_case_inputs_not_packaged_locally", [relation("DS1_DK1_MULTI_MARKET", "same_zone_post_reform_method_transfer", "The paper uses 2021 DK1 spot/activation data and projected 2024 aFRR rules; DS1 is observed post-reform 2025–2026 DK1, while the paper's wind-plant and hybrid-storage inputs are not local.")]),
    "M09": ("paper_agent_based_case_inputs_not_packaged_locally", [relation("DS3_RTS_GMLC", "storage_market_design_system_benchmark", "RTS-GMLC can test market-participation effects on simulated system cost and emissions; it does not reproduce the paper's North American agent-based cases.")]),
    "M10": ("review_no_single_original_dataset", [relation("DS3_RTS_GMLC", "market_simulation_guidance", "The review's market-design simulation requirements guide mechanism experiments; no single review dataset is reproduced.")]),
    "M11": ("review_no_single_original_dataset", [relation("DS3_RTS_GMLC", "forecast_dispatch_control_evidence_chain", "The synthetic system can connect forecasting, dispatch, and control layers, while field validation remains absent.")]),
    ...
    "D04": ("original_open_field_dataset_not_mirrored_locally", [relation("DS1_DK1_MULTI_MARKET", "field_deliverability_evidence_boundary", "DK1 supplies market drivers only; the Nature Energy field measurements show why simulated SOC feasibility cannot substitute for device telemetry and capacity-fade evidence.")]),
    ...
    "L07": ("original_storage_arbitrage_cases_not_packaged_locally", [relation("DS1_DK1_MULTI_MARKET", "online_storage_risk_control_transfer", "Transfers conformal risk-aware online control from single-market arbitrage to a sequential energy-reserve setting; the risk label and delayed settlement differ.")]),
    "L08": ("paper_application_datasets_not_packaged_locally", [relation("DS4_EPF_DE", "conformal_risk_training_transfer", "EPF-DE can support a battery-operation method transfer, but end-to-end risk training is reserved for P3 and does not reproduce the paper application.")]),
    "L09": ("generic_method_paper_no_energy_dataset", [relation("DS1_DK1_MULTI_MARKET", "conformal_decision_control_transfer", "The conformal-controller structure can target DK1 non-delivery loss, subject to a safe nested decision family and explicit delayed feedback.")]),
}
