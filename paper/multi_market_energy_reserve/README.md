...
论文核心问题是：在 aFRR 容量闸门早于日前能量闸门的固定时序下，一座价格接受者储能在 07:30 承诺的备用能否在交割日兑现。已有工作给出了分阶段随机决策（X14）、Copula 联合场景（X07）和单一实时能量市场中的在线储能代理风险控制（L07），但本库现有工作未直接覆盖后改制 DK1 的两闸门、实际 aFRR 未交付损失和结算延迟。S3-B 已表明概率场景能改善当前协议下的收益和聚合未交付，但复杂依赖结构没有稳定胜出，固定风险系数的名义尾部风险也未稳定兑现。因此 P1 待检验的主张是按已实现未交付事件在线控制风险，并在后改制 DK1 上按实际结算与同协议重建的已发表方法比较；预测评分与决策排名不一致已有 F07、F36、X20，P1 不主张；主张边界与对比批次见[论文规划](../../research-foundation-2026/PAPER_PLAN.md)第 6 节。
