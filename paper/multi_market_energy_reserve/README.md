...
论文核心问题是：在 aFRR 容量闸门早于日前能量闸门的固定时序下，一座价格接受者储能在 07:30 承诺的备用能否在交割日兑现。已有工作给出了分阶段随机决策（X14）与 Copula 联合场景（X07），但风险系数凭经验设定，场景独立抽样或静态，评估多为单日或模型内期望收益。P1 以闸门条件联合场景驱动三阶段决策、按已实现未交付事件在线校准风险系数，并在后改制 DK1 上按实际结算与同协议重建的已发表方法比较；主张边界与对比批次见[论文规划](../../research-foundation-2026/PAPER_PLAN.md)第 6 节。
