...
研究截止日：2026-09-11；重建日期：2026-09-10。
...
这里的“Top=45”由 25 篇锚定 Top 期刊和 20 篇正式国际顶会论文组成。交叉前沿的 X06 与 X09 分别处理激活交付概率保证和部分联合价格场景，但都没有同时联合预测本文三个目标；X14 已按 09:00 容量、12:00 日前、实时三阶段决策但场景取独立历史抽样，X07 的 Copula 场景中容量价与激活价为常数；F18--F33 是结构候选，不作为同问题对比对象。
...
论文核心问题是：在 aFRR 容量闸门早于日前能量闸门的固定时序下，一座价格接受者储能在 07:30 承诺的备用能否在交割日兑现。已有工作给出了分阶段随机决策（X14）与 Copula 联合场景（X07），但风险系数凭经验设定，场景独立抽样或静态，评估多为单日或模型内期望收益。P1 以按已实现未交付事件在线校准风险系数为主张，并在后改制 DK1 上按实际结算与同协议重建的已发表方法比较；闸门条件联合场景在预测层未获支持，决策层待 S3-B 裁定；预测评分与决策排名不一致已有 F07、X20，P1 不主张；主张边界与对比批次见[论文规划](../../research-foundation-2026/PAPER_PLAN.md)第 6 节。
