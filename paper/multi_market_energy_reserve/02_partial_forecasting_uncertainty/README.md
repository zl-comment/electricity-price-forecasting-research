...
| F15 | [源-荷-价多任务联合预测](01_top_journals/F15_SOURCE_2024_Applied_Energy_Source_Load_Price_MTL.md) | Top 期刊 / 来源卡 | 多任务预测结构参考；载体耦合，非跨市场功能耦合；P1 只作已冻结消融 |
...
| F31 | [Conformal PID](01_top_conferences/F31_2023_NeurIPS_Conformal_PID.pdf) | Top 国际会议 | 在线校准时序预测失覆盖；未测试储能未交付损失或至少两日延迟的决策反馈 |
...
| F34 | [Nature Communications System Forecasting](01_top_journals/F34_2026_Nature_Communications_Joint_System_Forecasting.pdf) | Top 期刊 | 负荷、光伏、风电的系统级联合概率预测及备用分配示例；不含储能竞价与交付 |
| F35 | [PriceFM](03_preprints/F35_2026_PriceFM.pdf) | 预印本 | 欧洲多地区、多步、多分位电价基础模型基线；截至截止日未正式同行评审 |
| F36 | [Foundation Models and Battery Value](03_preprints/F36_2026_Foundation_Models_Battery_Value.pdf) | 预印本 | 统计精度与电池套利价值排名错位的新证据；只覆盖日前单市场 |
| F37 | [Online Conformal EPF](01_top_journals/F37_2025_Applied_Energy_Online_Conformal_EPF.pdf) | Top 期刊 | 德国、意大利日前电价区间的在线重校准；校准对象仍是预测覆盖 |
| F38 | [Delayed-Feedback ACI](03_preprints/F38_2026_Delayed_Feedback_ACI.pdf) | 预印本 | 固定反馈延迟下 ACI 的覆盖界与延迟—记忆诊断；尚未验证储能决策损失 |

这些论文支持建立独立预测、跨市场特征、概率分布、校准、树模型和外生变量深度模型基线；它们不能单独证明储能备用承诺可交付。F01 与本地 D4 是精确公开基准关系，F08--F13 只是在 D4 上做同协议迁移评测，F18--F33 只作为结构候选与风险参照，F14 的广东原始数据尚未公开取得。F34 的备用分配是系统运营示例，F35、F36、F38 仍是预印本，F37 与 F31 校准的是预测失覆盖而非已实现未交付决策损失。
