...
它是 C1「联合预测优于独立预测」目前能找到的最接近的多任务证据，但**只支持方法层面，不支持设定层面**：这里的"多"是同一系统内的电/热/气载体与园区能源价格，不是日前电能量市场与 aFRR 容量市场这两个**市场功能**；共享层带来的增益来自载体间的物理耦合，而非跨市场的结算与时序耦合。引用时不得与 F02 的"多市场"混用——F02 的多市场指多价区。

因此本文不构成 C1 的同设定占位：能量—备用双市场的联合预测仍是空白，但也意味着本文的 MAPE 数字不能作为 P1 的对比基线。
