...
The market-only schedules overlap with only 0.0116--0.1409\% of midday proxy
energy.  On 206 complete forecast days, adding a gate-specific alignment credit
$\rho\in\{25,50,100\}$ EUR/MWh raises the pooled-Gaussian strategy's proxy
absorption from 0.0501\% to 0.0771\%, 0.1172\%, and 0.1906\%.  Relative to the
market-only control, mean realized profit changes by $+13.4$, $-18.0$, and
$-71.5$ EUR/day, respectively, while evening reserve and delivery change
little.  The signal therefore changes charge timing and 17:00 SOC, but does not
remove the physical scale limit or turn the proxy into measured curtailment.
\end{abstract}
...
shortfalls $\xi_s$.  The implemented CVaR representation is
\begin{align}
...
The proxy-aware arms add a gate-visible charge-alignment quantity without
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{0.27\textwidth}p{0.29\textwidth}p{0.32\textwidth}}
    ...
    Proxy-aware decision sensitivity
      & gate proxy forecast, market scenarios, planned charging
...
proxy-aware decision comparison uses the 206 days with complete 24-hour inputs
...
\begin{table}[htbp]
  \centering
  \small
  \caption{Market-only mean daily linkage outcomes on 206 common forecast-complete days.}
  \resizebox{\textwidth}{!}{%
  \begin{tabular}{lrrrrr}
    ...
    \midrule
    B2 FB0 & 6.212 & 6.881 & 18.463 & 97.38 & 481.5 \\
    B2 FB0 gate & 6.108 & 6.850 & 18.463 & 97.38 & 471.6 \\
    B3 historical paired, $\chi=0$ & 1.035 & 6.520 & 19.190 & 97.85 & 221.8 \\
    B3 historical paired, $\chi=0.5$ & 1.118 & 6.629 & 18.706 & 97.81 & 217.3 \\
    B3 pooled Gaussian, $\chi=0$ & 1.920 & 6.722 & 18.064 & 98.23 & 255.7 \\
    B3 pooled Gaussian, $\chi=0.5$ & 2.404 & 6.872 & 17.685 & 98.20 & 258.5 \\
    X06 rebuild & 2.695 & 6.884 & 18.648 & 97.68 & 321.6 \\
    X09 rebuild & 2.003 & 6.767 & 18.271 & 98.56 & 234.3 \\
    X14 rebuild & 0.513 & 5.971 & 19.612 & 97.51 & 215.9 \\
    \bottomrule
  \end{tabular}}
...
EUR 216--322/day.  B2 recovers less energy than several stochastic methods but
...
evening delivery rate, 98.56\%, while retaining moderate midday charging and
...
\subsection{Gate-proxy-aware decision sensitivity}

Table~\ref{tab:proxy-aware-results} compares the pooled-Gaussian market-only
control, which is the $\rho=0$ member of Eq.~\eqref{eq:proxy-aware-objective},
...
\begin{table}[htbp]
  \centering
  \small
  \caption{Proxy-aware sensitivity on 206 common forecast-complete days.}
  \resizebox{\textwidth}{!}{%
  \begin{tabular}{rrrrrrrr}
    \toprule
    $\rho$ & Midday charge & Forecast overlap & Realized overlap & SOC at 17:00
      & Evening reserve & Delivery & Realized profit change \\
    (EUR/MWh) & \multicolumn{5}{c}{MWh/day, except reserve in MW h/day}
      & (\%) & (EUR/day) \\
    \midrule
    0   & 1.920 & 0.615 & 0.775 & 6.722 & 18.064 & 98.23 & 0.0 \\
    25  & 2.441 & 1.141 & 1.194 & 6.784 & 18.014 & 98.24 & $+13.4$ \\
    50  & 3.418 & 2.112 & 1.815 & 6.869 & 17.948 & 98.24 & $-18.0$ \\
    100 & 5.232 & 4.038 & 2.951 & 6.967 & 17.955 & 98.24 & $-71.5$ \\
    \bottomrule
  \end{tabular}}
  \label{tab:proxy-aware-results}
\end{table}

Relative to $\rho=0$, the $\rho=25$ arm adds 0.521 MWh/day of midday charge,
0.526 MWh/day of forecast overlap, 0.419 MWh/day of realized overlap, and
0.062 MWh of 17:00 SOC.  The paired mean profit difference is
EUR $+13.4$/day with a 95\% interval of $[2.7,28.2]$ EUR/day.  At $\rho=50$,
realized overlap rises by 1.040 MWh/day but the profit difference is
EUR $-18.0$/day with interval $[-42.9,10.0]$.  At $\rho=100$, realized overlap
rises by 2.176 MWh/day and the profit difference is EUR $-71.5$/day with
...
the nine market-only strategies, the largest overlap is 449.5 MWh for B2 FB0.
...
34--44\%.  The proxy-aware arms raise that share to 48.9--56.4\%, and the
$\rho=100$ arm reaches
\begin{equation}
...
proxy forecast to an exploratory alignment objective increases forecast and
...
profit response is non-monotone: $\rho=25$ improves mean realized profit on
this sample, $\rho=50$ is statistically inconclusive, and $\rho=100$ lowers
...
\end{equation}
The market-only control sets $\rho=0$, so the forecast has no objective value;
the three sensitivity arms implement Eq.~\eqref{eq:proxy-decision-chain} through
Eqs.~\eqref{eq:forecast-overlap-proxy-bound}--\eqref{eq:proxy-aware-objective}.
...
market-only and three proxy-aware dispatch traces.
