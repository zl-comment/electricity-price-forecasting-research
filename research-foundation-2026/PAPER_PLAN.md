...
| 缺口 | 后果 | 处理 |
|---|---|---|
| **外部基线仍不覆盖概率预测** | F01 与 F08 已纳入 DK1 同协议评测，但二者均为小时级纯价格滞后模型，受 n/p=109/103=1.06 约束 | 概率预测侧外部基线定为 X07 式场景生成器、决策侧定为 X14 式三阶段随机优化，均未实现，见第 6 节；F08 在 R2 中做了部分复现（含验证集调参），Up 调参后仍接近常数预测器，按预设判据实现能力未获验证，见[锚定表](../p1_paper/results/anchor_table/anchor_table.csv) |
...
| 预测驱动 VPP 随机调度（X07） | 二级 | 负荷、可再生、日前价、激活比例各用 BLSTM 分位数回归，再以静态经验 Copula 抽样成联合轨迹；容量价与激活价为常数 | 12:00 能量与备用一次报价＋日内追索的两阶段随机 MILP＋CVaR | 比利时单个 7 月典型日，年份未给出 | 有条件：BLSTM 换线性分位数回归，Copula 在 84 日窗内估计，决策接入 07:30 三阶段结构 | 是 |
...
| 德国风电—电池 aFRR 竞价（X14） | 二级 | 容量价按 4 小时块历史抽样，日前价与激活价取历史实现值，激活为概率乘常数；风电场景保留时间相关 | 09:00 容量 → 12:00 日前 → 实时三阶段随机 MILP＋CVaR | 2022-07 至 2022-12，早于改制 | 是：附录公式完整；去掉风电，09:00 改 07:30，4 小时块改 1 小时 | 是 |
...
| CVaR 或事先设定置信水平的交付风险处理 | X06、X07、X13、X14 | 否 |
| 日前价以已公布容量价为条件、激活含路径、激活价随机的闸门条件联合场景 | 已核读候选中未见 | 是：以打乱跨目标配对与去掉容量价条件的消融检验其决策价值 |
| 按已实现未交付事件在线校准风险系数 | 已核读候选中未见 | 是：只保证测试期长期平均未交付率 |

"未见"只针对上表候选；Scopus、Web of Science、IEEE Xplore、Google Scholar、DTU Orbit 未检索，见下表。
