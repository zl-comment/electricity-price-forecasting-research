...
更新日期：2026-09-16。该目录围绕当前论文目标维护，数据角色分为：真实多市场主数据、备用预测外部基准、透明联合出清机制基准和公开电价预测基准。四套数据不得拼接成同一个真实物理系统。

“论文—数据一一对应”在本库中表示：84 篇/项文献中的每一项都有明确的 `original_data_status` 和至少一条本地数据关系；关系区分“论文精确公开数据”“同市场部分字段”“当前项目迁移基准”和“机制仿真基准”。这不表示为每篇论文复制一份相同 CSV，也不把未公开的原论文数据伪装成本地已有数据。正向映射见[论文—数据对应表](paper_data_alignment.json)，反向映射见[数据目录](catalog.json)，论文目录中也保存相同的 `data_links`。
...
| 数据 | 当前证据角色 |
|---|---|
| DK1 | 本分支的午间新能源富余充电、晚峰上调备用和实际激活结算主数据 |
| 未来 DK1 | 可用于未参与设计的确认性评价，不预设具体方法 |
| RTS-GMLC | 系统出清与机制验证；不进入本分支的实网主结论 |
| 芬兰 aFRR | 备用预测外部检验；不与 DK1 行级拼接 |
| EPF-DE | 预测基准；不含备用市场，不与 DK1 行级拼接 |

DK1 时序、缺失、可见性和共同标签窗口已经审计。点预测、概率场景和统一储能结算的既有实验已经使用 211 个测试日，该测试集不得在反复选择新结构后仍被表述为独立确认性证据。
...
- [数据审计](data_audit.json)：143 项论文/数据清单完整性校验、43 个 CSV 与 69 份 PDF 审计，以及 84 篇论文—4 套数据、89 条关系的双向对应检查。
