...
1. DK1 时序、缺失、可见性和共同标签窗口已审计并冻结；
2. 点预测、概率场景和统一储能结算的 S2/S3-A/S3-B 已完成，211 个测试日不再用于选择新预测结构；
3. 当前只用 DK1 冻结输入预注册并检验 S3-C 的延迟未交付风险控制；
4. 再用未参与设计的未来 DK1 或德国数据做确认性复验；
5. RTS-GMLC 用于系统出清与机制验证，芬兰用于备用预测外部检验，EPF-DE 用于精确预测基准和 P3 方法迁移；三者都不与 DK1 行级拼接。
